...
3. Workload Migration:
   ...
   - Executes pvenode migrateall with --with-local-disks 1.
...
5. Reconnection and Verification: Wait for the node to return to the network and verify Corosync quorum before proceeding to the next node.
...
- name: PVE | Evacuate VMs to {{ pve_migration_target }}
  ansible.builtin.command: "pvenode migrateall {{ pve_migration_target }} --with-local-disks 1"
  async: 1800
  poll: 10
  when:
